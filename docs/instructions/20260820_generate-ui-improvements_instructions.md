...
**変更:**
1. 位置を画面上部・ヘッダーの下に変更: `bottom: 62px` → `top: 56px`（ヘッダー高さの直下。ヘッダーと被らないこと）
2. 表示時間の3段階標準化（`AUTO_DISMISS_MS` を定数からtype別マップに変更）:
   - success（正常）: 2秒
   - info（注意）: 5秒
   - error（警告）: 10秒で自動消失（手動×も残す）。console.errorにもメッセージを出力する
3. フェード開始: 消失の300ms前（FADE_MS = 300 はそのまま）
4. flexDirectionを `column` に変更（上から下に積む。現在は下から上）
