...
| AC-3 | GitHub Pages が無効で、`gh-pages` ブランチが無い | ⚠️ 一部：ブランチ削除 ✅ ／ Pages の無効化は API で拒否 → **発注者操作が必要**（§8） | `git push origin --delete gh-pages` → `- [deleted] gh-pages`。`git ls-remote --heads origin gh-pages` → 空 ／ `gh api -X DELETE repos/misfortunemate-png/prompt-vault-dev/pages` → `{"message":"Deactivating GitHub pages for this repository is not allowed.","status":"422"}`。その後の GET は `"status":"built" … "source":{"branch":"gh-pages","path":"/"}` のまま |
...
| 条件 | 状態 |
|---|---|
| S-1〜S-5 を実施し、フロントを公開し直した | ✅（S-1 のうち Pages の無効化だけは発注者操作待ち） |
| 検証計画の全項目を実行し、証跡を添付 | ✅ §2 |
| inspect 緑・_STATUS.md 更新（S-5 様式）・pull・push | ✅ |

## 5. 未完了・未検証

- **GitHub Pages の無効化**（AC-3 の半分）。API が 422 で拒否した。`gh-pages` ブランチは消したので、ミラーの中身はもう更新されない。ただし Pages の設定は「built」のまま残っている（§8）
